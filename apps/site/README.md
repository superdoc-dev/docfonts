...
- **Registry-driven.** Pages read measured verdicts from `@docfonts/registry`. Until
  `scripts/import-research.ts` generates the canonical `records.json`, the site reads the explicit
  interim seed via `loadSeedRecords()` (a visible temporary dependency, not a silent fallback); it
  switches to `loadRecords()` once the importer lands. `src/pages/fonts/[font].astro` generates one
  canonical static page per proprietary font via `getStaticPaths` - the SEO surface. People search the
  proprietary name, so the title carries it.
