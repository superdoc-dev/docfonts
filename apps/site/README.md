...
- **Registry-driven.** Pages read measured verdicts from `@docfonts/registry` via `loadRecords()` -
  the canonical hand-curated registry (`data/registry/records.json`). `src/pages/fonts/[font].astro`
  generates one canonical static page per proprietary font via `getStaticPaths` - the SEO surface.
  People search the proprietary name, so the title carries it.
