...
It ships no fonts and no proprietary binaries: only the measured evidence (which open family stands in, the verdict, the advance delta, the license id, and a provenance pointer back to the docfonts registry).
...
- `evidenceId` - a provenance pointer back to the docfonts record.
...
The data is generated from the docfonts registry's reviewed substitution-evidence artifact. Every number reproduces from the registry's stated method against fonts you license yourself; docfonts distributes no proprietary binaries.
